...
| Custom pigment entry (Lab → synthesized spectrum) | `lib/features/pigments/custom_pigments_screen.dart` + `lib/features/pigments/custom_pigments_provider.dart`; `lib/engine/spectrum_from_lab.dart`, overlay `lib/engine/overlay_engine.dart`, Drift `CustomPigments` |
